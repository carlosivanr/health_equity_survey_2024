...
# Aurora Wellness Community Health Center. Options do not need to be in quotes
params:
  clinic: "Aurora Wellness Community Health Center (AWCHC)"
    
title: "DFM Health Equities Survey 2026"
...
# DFM Health Equities Surveys 2026
# New Clinic Added (Aurora Wellness Community Health Center (AWCHC))
# New demographic_race__7 icorporated in code
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
```
...
# Q2:
# In 2025, hi_clinic_* questions were not asked.
# In 2026, hi_clinic_* questions were asked only if clinic was Aurora Wellness
# hi_clinic_q1:hi_clinic_q4, hi_clinic_implicit
data %<>%
  mutate(across(c(hi_clinic_q1:hi_clinic_q4, hi_clinic_implicit),
...
```{r}
data <- data %>% filter(Year == 2026)

# Aurora Wellness Community Health Center (AWCHC)
```

```{r, eval = FALSE}
params <- list()
params$clinic <- "Aurora Wellness Community Health Center (AWCHC)"
```
...
data %>%
  ...
  tbl_summary(by = "Role",
              label = list(
                ...
                demographic_4_race___7 = "Middle Eastern or North African",
  ...
  modify_table_styling(columns = label,
                     ...
                     undo_text_format = "bold") %>%
  modify_table_styling(columns = label,
                     rows = variable == "demographic_4_race___7",
                     undo_text_format = "bold") %>%                    
  modify_table_styling(columns = label,
...
freq_prop_tab <- function(df, q) {
  ...
  bind_cols(role, df) %>%
    ...
    add_overall() %>%
    as_tibble() %>%
    write_csv(., str_c(here("2026/deliverables/csv"), "/", params$clinic, "_", q, ".csv"))
...
bind_cols(df, data.frame(Role = data$Role)) %>%
  ...
  add_overall() %>%
  as_tibble %>%
  write_csv(., str_c(here("2026/deliverables/csv"), "/", params$clinic, "_", "q6", ".csv"))
```
...
#   add_overall() %>%
#   as_tibble() %>%
#   write_csv(., str_c(here("2026/deliverables/csv"), "/", params$clinic, "_", "q7_2", ".csv"))
...
<!-- NOT ASKED IN 2025 , IN 2026 Only Aurora Community Wellness -->
...
#   modify_header(label = "") %>%
#   as_tibble() %>%
#   write_csv(., str_c(here("2026/deliverables/csv"), "/", params$clinic, "_", "q9", ".csv"))
```
...
# The following questions were presented to Aurora Wellness participants only for the 2026 survey 
## Among patients at your clinic, how much do you think each of the following contributes to health inequities?
```{r}
#| eval: true
data %>%
...
## Is there anything else that you think leads to health inequities among your patients?
```{r}
data %>%
  select(Role, patient_hi_q1) %>%
  mutate(patient_hi_q1 = fct_na_value_to_level(factor(patient_hi_q1), level = "Unknown")) %>%
  tbl_summary(
    by = Role) %>%
  modify_header(label = "")
...
## Please share what else you think leads to health inequities among your patients
```{r}
data %>%
  select(patient_hi_q2) %>%
  drop_na() %>%
  gt::gt()
```

## Which strategies do you think would be most effective in reducing health inequities among patients at yourclinic? (Select up to 3.)
```{r}
data_raw %>%
  filter(record_id %in% data$record_id, redcap_event_name == "2026_arm_1") %>%
  select(strategies___1:strategies___9) %>%
  mutate(across(strategies___1:strategies___9, ~ factor(.x, levels = c("0", "1")))) %>%
  bind_cols(., data %>%select(Role) )%>%
  tbl_summary(
    by = Role, 
    label = list(
      strategies___1 = "Practice leaders make a clear commitment to health equity and create a team to lead the work",
      strategies___2 = "Provide training on topics related to health equity, diversity, and inclusion",
      strategies___3 = "Focus on health inequities in quality improvement projects",
      strategies___4 = "Conduct a formal review to look for policies and practices that may unintentionally make health inequities worse",
      strategies___5 = "Provide data to staff and providers about health inequities among patients at this clinic",
      strategies___6 = "Ask patients about social barriers to health and refer them to resources (social barriers to health include problems with transportation, housing, food access, etc)",
      strategies___7 = "Partner with local community organizations to improve the health of your patients",
      strategies___8 = "Incorporate more input from patients experiencing health inequities into clinic decisions",
      strategies___9 = "Promote and celebrate diversity of clinic staff, clinicians, and leadership"
    ),
    value = everything() ~ "1")
```
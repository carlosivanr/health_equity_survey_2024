...
# DFM Health Equities Surveys 2024
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
```
...
data_lab <- pull_report('128507', 'label', 'raw') %>%
  select(-(contains("demographic_4_race")), -(contains("clinic_name____99")))

# Pull the 2024 report in raw format for the demographic race
...
# Drop record id 171 because it was a test record shown in clinic_name____99
data %<>%
...
col_labs <- names(
  (pull_report('128507', 'raw', 'label') %>% select(-"Clinic Name (choice=TEST RECORD)")))
```
...
```{r}
# Load 2023 data report ID = 128509
# Need to compare Q3 and Q4

# Pull the 2023 report in label format w/ out the demographic race columns and 
# clinic name
data_lab <- pull_report('128509', 'label', 'raw') %>%
  select(-(contains("demographic_4_race")), -(contains("clinic_name")))

# Pull the 2024 report in raw format for the demographic race and clinic_name
# columns only
data_raw <- pull_report('128509', 'raw', 'raw') %>%
  select(contains("demographic_4_race"), -(contains("clinic_name")))

# Merge the race cols into the main data frame
data_2023 <- bind_cols(data_lab, data_raw)
...
# Pull the column headers in label format for displaying in tables/charts
col_labs <- names(
  (pull_report('128509', 'raw', 'label') %>% select(-(contains("Clinic Name")))))

# Modify the column names by removing the _2023 and _v1 suffixes
new_col_names <-
  colnames(data_2023) %>%
  str_replace(., '_2023', replacement = '') #%>%
  # str_replace(., '_v1', replacement = '')

# Set the new column names to harmonize column names in R
colnames(data_2023) <- new_col_names

# Create a data frame with the column names and their corresponding column
# labels
labels_2023 <- data.frame(t(col_labs))
colnames(labels_2023) <- new_col_names
```
...
```{r}
# After processing data for q3 and q4, put back into a separate data frame so that the tables don't have to be modified
data_2023 <- data %>% 
  filter(Year == 2023)
...
# Response Rate
```{r}
# Lone Tree   --> clinic_name___1
# WISH        --> clinic_name___2
# Boulder     --> clinic_name___3
# AFW         --> clinic_name___4
# Westminster --> clinic_name___5
if (params$clinic == "Lone Tree") {
  data %<>%
    filter(clinic_name___1 == "Lone Tree")
  
  data_2023 %<>% 
    filter(Practice == "Lone Tree")
  
  
} else if (params$clinic == "WISH") {
  data %<>%
    filter(clinic_name___2 == "WISH")
  
  data_2023 %<>% 
    filter(Practice == "WISH")
  
} else if (params$clinic == "Boulder") {
  data %<>%
    filter(clinic_name___3 == "Boulder")
  
  data_2023 %<>% 
    filter(Practice == "Boulder")
  
} else if (params$clinic == "AF Williams") {
  data %<>%
    filter(clinic_name___4 == "AFW")
  
  data_2023 %<>% 
    filter(Practice == "AF Williams")
  
} else if (params$clinic == "Westminster") {
  data %<>%
    filter(clinic_name___5 == "Westminster")
  
  data_2023 %<>% 
    filter(Practice == "Westminster")
}
```
...
freq_prop_tab <- function(df, q) {
  ...
  bind_cols(role, df) %>%
    ...
    add_overall() %>%
    as_tibble() %>%
    write_csv(., str_c(here("deliverables/csv"), "/", params$clinic, "_", q, ".csv"))
...
<!-- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% -->
<!-- 2023 data is incomplete. 2023 vs 2024 comparisons not valid -->
<!-- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% -->
...
<!--   (data_2023 %>%  -->
<!--     filter(health_equity_in_dfm_practices_survey_complete == "Complete") %>% -->
<!--     select(Role, hi_individual_q1, Year)), -->
<!--   (data %>% -->
<!--     select(Role, hi_individual_q1, Year))) %>% -->
<!-- filter(Role != "Unknown Role") %>% -->
<!-- mutate(hi_individual_q1 = fct_na_value_to_level(hi_individual_q1, level = "Unknown")) %>% -->
<!-- rename("How important is it to you to address health inequities within your clinic?" = hi_individual_q1) -->
<!-- ``` -->
...
<!-- ) %>%  -->
<!--       as_tibble() %>% -->
<!--   write_csv(., str_c(here("deliverables/csv"), "/", params$clinic, "_", "q3_2", ".csv")) -->
<!-- ``` -->
...
<!--   (data_2023 %>% -->
<!--     filter(health_equity_in_dfm_practices_survey_complete == "Complete") %>% -->
<!--     select(Role, hi_clinicleaders_q1, Year)), -->
<!--   (data %>% -->
<!--     select(Role, hi_clinicleaders_q1, Year))) %>% -->
<!-- mutate(hi_clinicleaders_q1 = fct_na_value_to_level(hi_clinicleaders_q1, level = "Unknown")) %>% -->
<!-- rename("How important is it to your clinic leadership to address health inequities at your clinic?" = hi_clinicleaders_q1) -->
<!-- ``` -->
...
<!--       modify_header(label = "")) %>% -->
<!-- as_tibble() %>% -->
<!--    write_csv(., str_c(here("deliverables/csv"), "/", params$clinic, "_", "q4_2", ".csv")) -->
<!-- ``` -->
...
bind_cols(df, data.frame(Role = data$Role)) %>%
  ...
  add_overall() %>%
  as_tibble %>%
  write_csv(., str_c(here("deliverables/csv"), "/", params$clinic, "_", "q7", ".csv"))
```
...
bind_cols(df, role) %>%
  ...
  add_overall() %>%
  as_tibble() %>%
  write_csv(., str_c(here("deliverables/csv"), "/", params$clinic, "_", "q8_2", ".csv"))
...
bind_cols(df, data.frame(Role = data$Role)) %>%
  ...
  modify_header(label = "") %>%
  as_tibble() %>%
  write_csv(., str_c(here("deliverables/csv"), "/", params$clinic, "_", "q10", ".csv"))
```

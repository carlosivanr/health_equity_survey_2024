...
```{r}
# if (params$clinic == "AFWilliams") {
#   clinic_name <- "AF Williams"
# } else if (params$clinic == "LoneTree") {
#     clinic_name <- "Lone Tree"
# } else {
#   clinic_name <- params$clinic
# }
...
left_join(role, n_group, by = "Role") %>%
  ...
  select(-n_group) %>%
  bind_rows(., overall) %>%
  filter(hi_problems_q1 != "Unknown") %>%
  mutate(Role = factor(Role, levels = c("Unknown Role", "Staff", "Clinician/Provider", "Overall"))) %>%
  ggplot(aes(x = hi_problems_q1, y = Proportion, fill = Role)) +
  geom_col(alpha = 0.8, position = position_dodge2(width = 0.9, preserve = "single")) +
  theme_minimal()+
  scale_fill_brewer(type = "qual", palette = 2) +
  scale_y_continuous(labels = scales::percent,
                     limits = c(0,1)) +
  labs(y = "Percentage", x = "", title = "In the United States?") +
  coord_flip() +
...
overall <- data %>% select(Role, hi_problems_q1) %>% mutate(Role = 'Overall')
role <- data %>% select(Role, hi_problems_q1)

bind_rows(role, overall) %>%
  mutate(Role = factor(Role, levels = c('Overall', 'Clinician/Provider', 'Staff', 'Unknown'))) %>%
  ggplot(aes(x = hi_problems_q1, fill = Role, by = Role, y = after_stat(prop))) +
  geom_bar(alpha = 0.8, stat = "prop", position = position_dodge2(width = 0.9, preserve = "single")) +
  # geom_text(aes(label = round(after_stat(prop) * 100)),
  #           stat= "prop",
...
```{r, fig.width=6.5, dpi = 600}
overall <- data %>% select(Role, hi_problems_q2) %>% mutate(Role = 'Overall')
role <- data %>% select(Role, hi_problems_q2)

bind_rows(role, overall) %>%
  mutate(Role = factor(Role, levels = c('Overall', 'Clinician/Provider', 'Staff', 'Unknown'))) %>%
  ggplot(aes(x = hi_problems_q2, fill = Role, by = Role, y = after_stat(prop))) +
  geom_bar(alpha = 0.8, stat = "prop", position = position_dodge2(width = 0.9, preserve = "single")) +
  # geom_text(aes(label = round(after_stat(prop) * 100)),
  #           stat= "prop",
  #           position = position_dodge2(.9, preserve = 'single'),
  ...
  scale_y_continuous(labels = scales::percent,
                     limits = c(0,1)) +
  labs(y = "Percentage", x = "", title = "In the UC Health System?") +
  coord_flip() +
  theme(legend.position = "top") +
  guides(fill = guide_legend(title = ""))
```
...
```{r, fig.width=6.5, dpi = 600}
overall <- data %>% select(Role, hi_problems_q3) %>% mutate(Role = 'Overall')
role <- data %>% select(Role, hi_problems_q3)

bind_rows(role, overall) %>%
  mutate(hi_problems_q3 = fct_na_value_to_level(hi_problems_q3, level = "Unknown")) %>%
  mutate(Role = factor(Role, levels = c('Overall', 'Clinician/Provider', 'Staff', 'Unknown'))) %>%
  ggplot(aes(x = hi_problems_q3, fill = Role, by = Role, y = after_stat(prop))) +
  geom_bar(alpha = 0.8, stat = "prop", position = position_dodge2(width = 0.9, preserve = "single")) +
  # geom_text(aes(label = round(after_stat(prop) * 100)),
  #           stat= "prop",
  #           position = position_dodge2(.9, preserve = 'single'),
  ...
  scale_y_continuous(labels = scales::percent,
                     limits = c(0,1)) +
  labs(y = "Percentage", x = "", title = "In your clinic?") +
  coord_flip() +
  theme(legend.position = "top") +
  guides(fill = guide_legend(title = ""))
```
...
```{r}
all_apply_tab <- function(df, sort = FALSE) {
  #' all_apply_tab
  #'
...
bind_rows(overall, role) %>%
  ...
  #           hjust = -0.5) +
  scale_y_continuous(labels = scales::percent,
                     limits = c(0,1)) +  guides(fill = guide_legend(title = "")) +
  scale_fill_brewer(type = "qual", palette = 2) +
  labs(x = "") +
  theme_minimal() +
  theme(legend.position = "top") +
...
new_col_names <- c("KnowledgeSkills",
               "ClinicCulture",
                "UCHCulture",
                "InequityAwareness",
                "Desire",
...
plot(likert_sw) +
# plot(likert_sw, group.order = new_col_names) +
  # scale_fill_manual(values = rev(RColorBrewer::brewer.pal(n = 3, "RdBu"))) +
  labs(title = "Overall") +
  theme_minimal() +
